...
class ks_gini_cap30_AUC():
    ...
    def calculate_gini(self,num = 1):
        ...
        for i in range(1,11): #計算Gini
            if i == 1:
                gini_list.append((sum(df.sort_values(by = columns_prob[num],ascending=False)[:round(len(df)/10)]['y'])/ \
                                        sum(df['y']))*0.1)
            else:
                gini_list.append((sum(df.sort_values(by = columns_prob[num],ascending=False)[round(len(df)*(i-1)/10):round(len(df)*i/10)]['y'])/ \
                                  sum(df['y']))*(2*i-1)/10)
